    ...
    <div className="h-screen flex flex-col bg-white font-sans">
      ...
      <div className="flex-1 flex pt-16 overflow-hidden">
        ...
        <main
          ref={mainRef}
          className={cn(
            "flex-1 p-4 md:p-8 bg-white transition-all duration-300 overflow-y-auto"
          )}
      ...
      <div className="fixed bottom-8 right-8 z-50 flex flex-col space-y-2">
        ...
        {showGoToTop && viewMode === 'list' && <Button onClick={handleGoToTop} size="icon" className="bg-primary text-primary-foreground rounded-full shadow-lg hover:bg-primary/90"><ArrowUp className="w-6 h-6" /></Button>}
      </div>
    </div>
  );
}
