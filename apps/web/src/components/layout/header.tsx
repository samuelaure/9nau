        ...
        <header className={cn(
            ...
            <div className="flex-1 mx-8">
                <div className="relative max-w-2xl mx-auto">
                    ...
                    <Input type="text" placeholder="Search..." className="w-full bg-gray-100 border-transparent rounded-lg pl-12 pr-4 focus:bg-white focus:shadow-md" />
                </div>
            </div>
            <div className="flex items-center space-x-1">
                {currentView === 'home' && (
                    <>
                        <Button variant="ghost" size="icon" onClick={() => setViewMode('list')} className={cn(viewMode === 'list' && 'bg-slate-200')}>
                            <List className="h-5 w-5 text-gray-600" />
                        </Button>
                        <Button variant="ghost" size="icon" onClick={() => setViewMode('horizontal')} className={cn(viewMode === 'horizontal' && 'bg-slate-200')}>
